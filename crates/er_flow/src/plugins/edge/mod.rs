...
impl Plugin for EdgePlugin {
    ...
    fn on_event(
        ...
        if let FlowEvent::Input(crate::plugin::InputEvent::MouseDown(ev)) = event {
            ...
            if current_tool_mode(ctx.shared_state).is_pan() {
                return crate::plugin::EventResult::Continue;
            }
    ...
    fn render(&mut self, ctx: &mut crate::RenderContext) -> Option<gpui::AnyElement> {
        ...
        let edges: Vec<_> = edge_ids
            .iter()
            .filter_map(|id| {
                let edge = ctx.graph.edges.get(id)?;
                Some((*id, edge_geometry2(edge, &ctx)))
            })
            .collect();
...
fn hit_test_get_edge(mouse: Point<Pixels>, ctx: &mut PluginContext) -> Option<EdgeId> {
    let visible_nodes: HashSet<_> = ctx
        .graph
        .nodes()
        .iter()
    ...
    for edge_id in edge_ids {
        let Some(edge) = ctx.graph.edges.get(&edge_id) else {
            continue;
        };
